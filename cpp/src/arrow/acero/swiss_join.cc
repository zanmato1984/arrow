...
Status SwissTableForJoinBuild::PartitionBatch(size_t thread_id, int64_t batch_id,
  ...
  auto num_rows = key_batch.length;

  // Compute hash
  //
  batch_state.hashes.resize(num_rows);
  RETURN_NOT_OK(Hashing32::HashBatch(
      key_batch, batch_state.hashes.data(), locals.temp_column_arrays, hardware_flags_,
      temp_stack, /*start_row=*/0, static_cast<int>(num_rows)));
