...
int RowArrayAccessor::VisitNulls_avx2(const RowTableImpl& rows, int column_id,
  ...
  int null_bits_per_row = 8 * rows.metadata().null_masks_bytes_per_row;
  ...
  for (int i = 0; i < num_rows / kUnroll; ++i) {
    ...
    auto [row_id_lo, row_id_hi] = util::ConvertInt32ToInt64(row_id);
    auto bit_id_lo = util::MulAddInt64(row_id_lo, null_bits_per_row, pos_after_encoding);
    auto bit_id_hi = util::MulAddInt64(row_id_hi, null_bits_per_row, pos_after_encoding);
    ...
    auto [result_lo, result_hi] = util::ConvertInt32ToInt64(result);
    ...
    uint32_t null_bytes_lo = static_cast<uint32_t>(_mm256_movemask_epi8(result_lo));
    uint64_t null_bytes_hi = _mm256_movemask_epi8(result_hi);
