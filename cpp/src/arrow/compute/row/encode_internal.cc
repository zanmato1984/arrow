    ...
    switch (col_prep.metadata().fixed_length) {
      ...
      case 2:
        for (uint32_t i = 0; i < num_rows; ++i) {
          ...
          // reinterpret_cast<uint16_t*>(col_base)[i] =
          //     *reinterpret_cast<const uint16_t*>(rows.fixed_length_rows(start_row +
          //     i));
        }
        break;
      case 4:
        for (uint32_t i = 0; i < num_rows; ++i) {
          ...
          // reinterpret_cast<uint32_t*>(col_base)[i] =
          //     *reinterpret_cast<const uint32_t*>(rows.fixed_length_rows(start_row +
          //     i));
        }
        break;
      case 8:
        for (uint32_t i = 0; i < num_rows; ++i) {
          ...
          // reinterpret_cast<uint64_t*>(col_base)[i] =
          //     *reinterpret_cast<const uint64_t*>(rows.fixed_length_rows(start_row +
          //     i));
        }
        break;
      default:
